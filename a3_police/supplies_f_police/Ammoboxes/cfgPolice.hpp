...
class Box_POLICE_Equip_F: Box_NATO_Equip_F
{
	...
	{
		item_xx(U_I_P_PoliceUniform_F,3);
		item_xx(U_I_P_PoliceUniform_gloves_F,2);
		item_xx(V_TacVest_blk_POLICE,3);
		item_xx(V_CarrierRigKBT_01_light_POLICE_F,2);
		item_xx(H_Cap_police,2);
		item_xx(H_Beret_blk_POLICE,1);
		item_xx(H_HelmetSpecter_black_headset_F,2);
		item_xx(FirstAidKit,10);
		item_xx(acc_flashlight,5);
		item_xx(I_P_UavTerminal,1);
		item_xx(Binocular,1);
		item_xx(Medikit,1);
